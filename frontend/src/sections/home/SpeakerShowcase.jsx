...
const SpeakerShowcase = () => {
    ...
    useEffect(() => {
        const getSpeakerProducts = async () => {
            try {
                const allProducts = await fetchProducts();
                const speakerProducts = allProducts.filter(
                    (product) => product.category === "speaker"
                );

                if (speakerProducts.length > 0) {
                    // Shuffle the speaker products and get the first 6
                    const shuffledProducts = speakerProducts.sort(() => 0.5 - Math.random());
            ...
            >
                <div className="text-center py-10">
                    <h2 className="text-xl font-medium text-gray-700 mb-2">
                        No speaker products found
                    </h2>
                    <p className="text-gray-500 mb-4">
                        We couldn't find any products in the Speaker category at the moment.
                    </p>
